...
- (void)viewDidLoad {
    ...
    [[self LaboratorioTableView]setDataSource:self];    // Do any additional setup after loading the view.
}
...
//   ----- Metodos heredados de tabla ----- consulta de datos
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = [NSString stringWithFormat:@"%@ %@", alaboratorio.cupo, alaboratorio.planta];
    cell.detailTextLabel.text = [NSString stringWithFormat:@"%@",alaboratorio.idTipoLab];
    return cell;
}
...
- (IBAction)InsertarLaboratorioBoton:(id)sender {
    ...
    if (sqlite3_open([[controlBD sharedInstance].dbPath UTF8String], &reservaBD)==SQLITE_OK){
        ...
        if(sqlite3_exec(reservaBD,insert_stmt,NULL,NULL,&error)==SQLITE_OK){
            NSLog(@"Laboratorio insertado correctamente");
...
- (IBAction)ActualizarLaboratorioBoton:(id)sender {
    ...
    if (sqlite3_open([[controlBD sharedInstance].dbPath UTF8String], &reservaBD) == SQLITE_OK) {
        char *update_Stmt = "update laboratorio set idTipoLab=? and cupo=? and planta=? where codLab=?";
        if (sqlite3_prepare(reservaBD, update_Stmt, -1, &statement, NULL) == SQLITE_OK) {
            sqlite3_bind_text(statement, 1, [self.IdTipoLabField.text UTF8String], -1, SQLITE_TRANSIENT);
            sqlite3_bind_text(statement, 2, [self.cupoField.text UTF8String],-1,SQLITE_TRANSIENT);
            sqlite3_bind_text(statement, 3, [self.plantaField.text UTF8String],-1, SQLITE_TRANSIENT);
            ...
            NSLog(@"Laboratorio modificado");
        }else{
            NSLog(@"Laboratorio NO modificado");
        }
        sqlite3_close(reservaBD);
    }
}
...
//   ----- Agregar y modificar metodo heredado
...
//    ----- teclado se oculta al seleccionar otro objeto
